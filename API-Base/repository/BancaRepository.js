...
const sequelize = require('../components/Connection.js');
let initModels = require("../src/modelAppBanca");
...
let UserRepository = function () {
    ...
    // Funcion EstadodeCuentaMonetaria
const EstadodeCuentaMonetaria = async (ConsulNoTarjeta, fechaInicio, fechaFin) => {
    try {
      // Buscar movimientos en la tabla 'movimiento' basados en las fechas y el número de tarjeta
      ...
      throw new Error(`Error al obtener el estado de cuenta monetaria: ${error.message}`);
    }
  };

//Consulta de Saldos TC
let getConsultaSaldosTC = async (CodTarjetaConsulta,month ) => {
        return await db.appbancabd.tarjeta.findAll({
            where: {
                id_tarjeta:CodTarjetaConsulta,
                fecha_pago:month,
                
                //empresa_id: params.company_id,
                //code: KUSTOMER_LOCATION_CODE
            }
        });
    }
